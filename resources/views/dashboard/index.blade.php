    ...
    <div class="app-page space-y-6">
        <div x-data="{ open: false }" class="app-card p-5">
            ...
            <form x-show="open" x-transition class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6" method="GET" action="{{ route('dashboard') }}">
                ...
                <label class="text-xs font-semibold text-slate-500 md:col-span-2 lg:col-span-2 xl:col-span-2">
                    Usuario
                    <select class="app-input mt-1" name="user_id">
                        <option value="">Todos</option>
                    ...
                    <table class="app-table">
                        ...
                        <tbody class="divide-y divide-slate-200/70">
                            @forelse($rankingUsuarios as $r)
                                <tr>
                                    <td class="font-semibold text-slate-900">{{ optional($r->usuario)->name ?? 'Usuario' }}</td>
                                    <td class="text-right font-semibold">{{ $r->cantidad }}</td>
                                    <td class="text-right font-semibold">$ {{ number_format($r->total, 2, ',', '.') }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="px-6 py-4 text-sm text-slate-500">Sin datos para este período.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
